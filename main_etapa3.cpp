...
public:
    ...
    Bikes& operator[](int index){
        if(index > this->nr){
            cout << "nu se poate! UwU";
        }
...
public:
    ...
    friend ostream& operator<<(ostream& out, const Employs& nenea){
        ...
        if(nenea.privacy == true){
            throw "**Acest angajat nu doreste sa ii fie vizualizate aceste date(salariul si rating-ul)**;\n";
        }else
        out << nenea.salariu << ";\n";

        out << "Rating from costumers(from 1 to 10): ";
        if(nenea.privacy == true){
            throw "**Acest angajat nu doreste sa ii fie vizualizate aceste date**;\n";
        }
...
public:
    ...
    virtual void addBike(Bikes &bike){
        ...
        if(this->buget > bike.getPrice()){
            ...
            throw "Bugetul dumneavoastra este insuficient! \n";
        }
...
int main()
{
    ...
    //cout << da;
                    ...
                    ///            ***LIST***        ///
    list <Employs> employList;
    employList.push_back(Gigel);
    employList.push_back(Ionel);

    for (list<Employs>::iterator it = employList.begin(); it != employList.end(); it++){
        try{
            cout << *it;
            cout << endl;
        }
